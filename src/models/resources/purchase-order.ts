// Cin7 Omni API — PurchaseOrder + PurchaseOrderItem
// Source: https://api.cin7.com/api/Help/ResourceModel?modelName=PurchaseOrder
import type { AccountingAttributes } from "./common";

/** A line item on a {@link PurchaseOrder}. */
export interface PurchaseOrderItem {
  ...
  Barcode?: string; // Product identifier
  ...
  QtyShipped?: number; // Quantity dispatched (Read-only)
  HoldingQty?: number; // Holding Quantity (Read-only)
  AccountCode?: string; // Alternative GL account code
}

/** A Cin7 purchase order (`PurchaseOrder` resource model). */
export interface PurchaseOrder {
  ...
  BranchId?: number; // Defaults to Main Branch ID
  ...
  Total?: number; // Incl. product, freight, discount, surcharge, and tax totals
  CurrencyCode?: string; // Three-character ISO currency code
  CurrencyRate?: number; // Rate used for conversion (auto-lookup if omitted)
  ...
  SupplierInvoiceReference?: string; // Reference from supplier invoice
  SupplierAcceptanceDate?: string; // Date of supplier acceptance (UTC)
  Port?: string; // Port for Indent Order
  EstimatedArrivalDate?: string; // Estimated time of arrival for Indent Order (UTC)
  FullyReceivedDate?: string; // Date when order was fully received (UTC)
  InvoiceDate?: string; // Date of invoice (UTC)
  LineItems?: PurchaseOrderItem[]; // Collection of purchase order line items
  ...
  SalesPersonEmail?: string; // Obsolete; returns null
...
/** Create/update payload for {@link PurchaseOrder} (Id is server-assigned on create). */
export type PurchaseOrderInput = Partial<PurchaseOrder>;
